class Laser {
    ...
    render(context) {
        ...
        context.fillRect(this.x + this.width * 0.2, this.y, this.width * 0.6,
            this.height);
        context.restore();
...
class Player {
    ...
    draw(context) {
        ...
        context.drawImage(this.jets_image, this.jetsFrame * this.width, 0, this.width,
...
class Enemy {
    ...
    update(x, y) {
        ...
        //check collosion enemies - projectiles
        this.game.projectilesPool.forEach(projectile => {
            if (!projectile.free && this.game.checkCollision(this,
                projectile) && this.lives > 0) {
                this.hit(1);
                projectile.reset();
            }
        });
        ...
        //lose 
        if (this.y + this.height > this.game.height || this.game.player.lives <
            1) {
...
class Boss {
    ...
    update() {
        ...
        this.x += this.speedX;
        this.y += this.speedY;
        //collsion detection boss/ projectile
        this.game.projectilesPool.forEach(projectile => {
            if (this.game.checkCollision(this, projectile) && !projectile.free && this.
                lives >= 1 && this.y >= 0) {
                this.hit(1);
                projectile.reset();
            }
        })
        //collision deteciton boss/player
...
class Game {
    constructor(canvas) {
        ...
        this.shootingInterval = null; // thêm dòng này
    ...
    render(context, deltaTime) {
        ...
        this.bossArray = this.bossArray.filter(object => 
    ...
    //collision detetion between 2 rectangles
    checkCollision(a, b) {
        return (
            a.x < b.x + b.width &&
            a.x + a.width > b.x &&
